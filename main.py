...
# ---------- Global crash handler: write to file + show on-screen ----------
...
# ---------- Kivy imports wrapped so import errors get displayed ----------
_kivy_import_error = None
try:
    from kivy.config import Config
    # Force fullscreen / no titlebar before any Window import
    ...
    _kivy_import_error = traceback.format_exc()
    try:
        with open(CRASH_FILE, "a") as f:
            f.write("==== KIVY IMPORT ERROR ====\n" + _kivy_import_error + "\n")
    except Exception:
        pass
...
# Everything below this line requires Kivy. Wrap in a guard so we can
# still display errors on Android if the import failed above.
if _kivy_import_error is None:
    ...
    class BgBox(BoxLayout):
        ...
        def __init__(self, **kw):
            ...
            self.bind(pos=self._sync, size=self._sync,
                      bg_color=self._sync_color, radius=self._sync_radius)
    ...
    class ChatBubble(BgBox):
        def __init__(self, text, role="user", **kw):
            ...
            # NOTE: do NOT set font_name on Android — RobotoMono isn't bundled
            # by python-for-android by default and a missing font crashes Kivy.
            self.lbl = lbl
            self.add_widget(lbl)
    ...
    class ErrorScreen(Screen):
        def __init__(self, message, **kw):
            ...
            root.bg_color = BG
            title = Label(text="App Error", color=ACCENT, font_size="22sp",
                          bold=True, size_hint_y=None, height=dp(50))
            scroll = ScrollView(size_hint=(1, 1), bar_width=dp(3),
                                bar_color=ACCENT)
            body = Label(text=message, color=TEXT_FG, font_size="12sp",
                         halign="left", valign="top", size_hint_y=None,
                         markup=False)
            body.bind(width=lambda i, v: setattr(i, "text_size", (v - dp(8), None)))
            ...
            close_btn.bind(on_release=lambda *_: App.get_running_app().stop())
    ...
    class AuthScreen(Screen):
        ...
        def __init__(self, **kw):
            ...
            self.code_lbl = Label(text="", color=ACCENT, font_size="34sp",
            ...
            self.copy_btn = Button(text="Copy Code", size_hint_y=None,
                                   height=dp(44), background_normal="",
                                   background_color=(0.1, 0.15, 0.2, 1),
                                   color=ACCENT, font_size="14sp",
                                   disabled=True)
        ...
        def copy_code(self, *_):
            try:
                ...
                self.status_lbl.text = f"Copy failed: {e}"
        ...
        def _on_fail(self, msg):
            self.status_lbl.text = "Failed: " + str(msg)[:300]
        ...
        def _auth_worker(self):
            try:
                ...
                verify_url  = data.get("verification_uri",
                                       "https://github.com/login/device")
                interval    = int(data.get("interval", 5))
                expires_in  = int(data.get("expires_in", 900))

                self._set_codes(user_code, verify_url,
                                "Open the URL on any device, enter the code, and approve.")
                ...
                self._on_fail(f"{type(e).__name__}: {e}")
    ...
    class ChatScreen(Screen):
        def __init__(self, **kw):
            ...
            clear_btn = Button(text="Clear", size_hint_x=None, width=dp(70),
                               background_normal="",
                               background_color=(0.15, 0.1, 0.12, 1),
            ...
            self.scroll = ScrollView(size_hint=(1, 1), bar_width=dp(3),
                                     bar_color=ACCENT,
                                     bar_inactive_color=(0, 0.5, 0.7, 0.4))
            ...
            inputbar = BgBox(orientation="horizontal", size_hint_y=None,
                             height=dp(64),
    ...
    class AIChatApp(App):
        ...
        def build(self):
            try:
                ...
                # Fullscreen + immersive on Android
                try:
                    Window.fullscreen = "auto"
                except Exception:
                    pass
                self._enable_android_immersive()

                self.gh_token = load_gh_token()
                self.copilot = (CopilotSession(self.gh_token)
                                if self.gh_token else None)
                ...
                try:
                    ...
                    pass
                sm = ScreenManager()
                sm.add_widget(ErrorScreen(message=tb, name="err"))
                sm.current = "err"
                return sm

        def _enable_android_immersive(self):
            """Hide status + nav bars on Android (immersive sticky)."""
            try:
                ...
                # Not on Android, or pyjnius missing — ignore
                pass
    ...
    # Kivy itself failed to import — try a barebones display so the user
    # sees what went wrong instead of a silent crash.
    def _main():
        print("KIVY IMPORT ERROR:\n" + (_kivy_import_error or "unknown"))
        try:
            with open(CRASH_FILE, "a") as f:
                f.write("KIVY IMPORT ERROR:\n" + (_kivy_import_error or "") + "\n")
        except Exception:
            pass
